...
def feedforward(input_net):
    ...
    x1 = np.dot(input_net , w1.T)  #input_net(1,9)     w1(8,9)
    y1 = activation(x1)
    x2 = np.dot(y1 , w2.T)
    y2 = activation(x2)
    x3 = np.dot(y2 , w3.T)
...
#calculating MSE
    ...
    print('epoch', i ,'MSE train = ' ,mse_train,'\taccurcy train = ',acc_train)

    # calculating mse and acc for val
    ...
    print('epoch', i, 'MSE val = ', mse_val, '\taccurcy val = ', acc_val)
    print("---------------------------------------------------------------------------------------")
